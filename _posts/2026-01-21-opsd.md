...
We evaluate OPSD on competition-level mathematical reasoning benchmarks (AIME 2024/2025, HMMT 2025) using the Qwen3 model family (1.7B, 4B, and 8B parameters). In the main results below, SFT, GRPO, and OPSD all used the same training datasets from OpenThoughts. For GRPO, we used a 16k generation length and sampled 8 rollouts per problem, while for OPSD, we used only a 1024 generation length for distillation and sampled only 1 rollout per problem. Our results show that across 3 model scales, OPSD has proven to be better than SFT and matches or exceeds GRPO while being significantly more token-efficient. SFT performance degrades because the concise reasoning solutions in OpenThoughts reduce generation length at test time; OPSD transforms these same solutions into dense token-level supervision through rationalization instead. GRPO stagnates due to reward diversity collapse — within 100 steps, more than half of its batches have zero reward standard deviation, yielding no gradient signal even while continuing to sample expensive rollouts.
...
2. Reduced generation budgets (1k tokens versus 16k for GRPO). We hypothesize that the **early tokens are more important for distillation than the later tokens, as the earlier tokens can represent more important branching points**.

In practice, this translates to **reduced training time** and **lower computational requirements**.

## Discussions

### Generation Style and Per-Token KL Clipping

A key design choice in OPSD is the generation style pairing between student and teacher. Qwen3 models support Thinking Mode on (TM-on), where the model produces chain-of-thought tokens, and Thinking Mode off (TM-off), where it responds directly. Among all four student/teacher pairings, the **TM-off student paired with a TM-on teacher** yields the largest KL divergence on math-related tokens, indicating stronger supervision on mathematically relevant content, and achieves the best downstream performance.

However, token-level divergence is highly skewed across vocabulary entries: a small subset of stylistic tokens (e.g., reasoning connectives like "therefore", "however") exhibits much higher divergence than mathematically meaningful tokens. Without correction, these stylistic tokens dominate the training signal. We address this with **per-token pointwise KL clipping**, which caps the per-vocabulary-entry divergence contribution at each position. As shown in Figure 4, clipping stabilizes training and prevents performance collapse — particularly important given that OPSD converges rapidly within a hundred steps.
...
<div class="caption">
    Ablation on using per-token pointwise KL clipping.
</div>
